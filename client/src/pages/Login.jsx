...
const loginSchema = yup.object({
    ...
    password:yup.string().length(6,"Password is exactly 6 length").required("password is required")
});
...
const Login = () => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 w-xl rounded-4xl shadow-2xl  mt-10 border-2 lg:px-8">
      <div className="mx-auto max-w-lg text-center ">
        <h1 className="text-2xl font-bold sm:text-3xl">Welcome Back!</h1>
        <p className="mt-4 text-gray-600">
        Sign in to access your daily quotes.
        </p>
      </div>
      <form noValidate className="mx-auto mb-0 mt-8 max-w-md space-y-4" onSubmit={handleSubmit(Submit)}>
        <div>
        ...
        <div>
          ...
          <div className="relative">
            <input placeholder="Enter your password" {...register("password")} className="w-full rounded-lg border-gray-300 p-4 pe-12 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent" id="password" type="password" />
            {errors.email && <p className='text-red-500'>{errors.email.message}</p>}
            <span className="absolute inset-y-0 end-0 grid place-content-center px-4">
              <svg stroke="currentColor" viewBox="0 0 24 24" fill="none" className="h-6 w-6 text-gray-400" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-600">
            No account yet?
            <a href="#" className="underline"><Link to="/signup">Create one</Link></a>
          </p>
